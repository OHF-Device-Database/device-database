/* eslint-disable @typescript-eslint/no-floating-promises, @typescript-eslint/no-unnecessary-condition, @typescript-eslint/restrict-plus-operands -- ignored in original code */
...
export class Routes implements ReactiveController {
	...
	private _status?: ((code: number) => void) | undefined;
	...
	initialGoto(pathname: string) {
		...
		if (this.routes.length === 0 && this.fallback === undefined) {
			...
			if (route === undefined) {
				throw new RouterPathNotFoundError(pathname);
			}
...
/* eslint-enable @typescript-eslint/no-floating-promises, @typescript-eslint/no-unnecessary-condition, @typescript-eslint/restrict-plus-operands -- ↑ */
