...
const {
  values: { "docs-directory": docsDirectory, "categories-file": categoriesFile, out },
} = args;
...
const categoryIdentifiers: Set<string> = new Set();
{
  ...
  const parsed: Record<string, Category> = JSON.parse(await readFile(categoriesFile, "utf-8"));
...
{
  let valid = true;
  for (const [integration, categories] of Object.entries(integrationCategoryMapping)) {
...
for await (const path of glob(`${integrationsDir}/*.markdown`)) {
  ...
  if (typeof parsed.ha_supporting_domain !== "undefined" && parsed.ha_supporting_domain !== name) {
  ...
  if (typeof integrationCategoryMapping[name] !== "undefined") {
    ...
    for (const [keySet, classification] of haCategoryMapping) {
      if (keySet.size === categories.length && categories.every((c) => keySet.has(c))) {
...
console.log("[*] explicitly ignored integrations that were never encountered →", [
  ...integrationExcluded.difference(integrationEncountered),
]);
console.log("[*] explicitly mapped integrations that were never encountered →", [
  ...new Set(Object.keys(integrationCategoryMapping)).difference(integrationEncountered),
]);
console.log("[*] explicitly mapped integrations that were never excluded →", [
  ...new Set(Object.keys(integrationCategoryMapping)).intersection(integrationExcluded),
]);
