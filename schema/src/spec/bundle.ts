...
if (typeof values.spec === "undefined") {
	console.error(
		"required parameter '--spec' missing (location of OpenAPI specification)",
	);
	process.exit(1);
}
if (typeof values.out === "undefined") {
	console.error(
		"required parameter '--out' missing (destination for bundled specification)",
	);
	process.exit(1);
}
